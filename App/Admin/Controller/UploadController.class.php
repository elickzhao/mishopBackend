...
class UploadController extends PublicController
{
    ...
    public function contentImage()
    {
        ...
        if (IS_POST) {
            $info = $this->upload_images($_FILES, array('gif','jpg','png','jpeg'), "product/".$_REQUEST['flag']);
            if (!is_array($info)) {// 上传错误提示错误信息
                ...
                foreach ($info as $key => $value) {
                    $data[] = '/Data/UploadFiles/'.$value['savepath'].$value['savename'];
                }
                $this->ajaxReturn([errno=>0,data=>$data]);
            }
        } else {
